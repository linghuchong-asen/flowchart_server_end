...
export class SummaryMemory {
  ...
  constructor(fields: SummaryMemoryInput) {
    this.kvStore = fields.kvStore;
    this.sessionId = fields.sessionId;
    this.summaryRepository = fields.summaryRepository;
  }
  ...
  async getSummary(): Promise<string> {
    ...
    if (this.summaryRepository) {
      try {
        ...
        console.warn('Failed to get summary from database, falling back to kvStore:', error);
      }
    }
  ...
  async setSummary(summary: string, projectId?: string): Promise<void> {
    ...
    if (this.summaryRepository) {
      try {
        ...
        console.warn('Failed to save summary to database, falling back to kvStore:', error);
      }
    }
  ...
  async saveContext(
    ...
    await this.setSummary(summary, projectId);
  }
  ...
  async clear(): Promise<void> {
    ...
    if (this.summaryRepository) {
      try {
        ...
        console.warn('Failed to clear summary from database, falling back to kvStore:', error);
      }
    }
